...
for i = 1 : nexp
 ...
 if (strcmp(bc_type,"ustrain"))
   for n = 1 : size(bc_x0, 1)
     u([bc_x0(n)*dim - 1, bc_x0(n)*dim + 0]) = [strain_exp(i,1) strain_exp(i,3)/2 ; strain_exp(i,3)/2 strain_exp(i,2)] * coordinates(bc_x0(n), :)';
     u([bc_x1(n)*dim - 1, bc_x1(n)*dim + 0]) = [strain_exp(i,1) strain_exp(i,3)/2 ; strain_exp(i,3)/2 strain_exp(i,2)] * coordinates(bc_x1(n), :)';
   end
   for n = 1 : size(bc_y0, 1)
     u([bc_y0(n)*dim - 1, bc_y0(n)*dim + 0]) = [strain_exp(i,1) strain_exp(i,3)/2 ; strain_exp(i,3)/2 strain_exp(i,2)] * coordinates(bc_y0(n), :)';
     u([bc_y1(n)*dim - 1, bc_y1(n)*dim + 0]) = [strain_exp(i,1) strain_exp(i,3)/2 ; strain_exp(i,3)/2 strain_exp(i,2)] * coordinates(bc_y1(n), :)';
   end
   u([X0Y0_nod*dim - 1, X0Y0_nod*dim + 0]) = [strain_exp(1,i) strain_exp(3,i)/2 ; strain_exp(3,i)/2 strain_exp(2,i)] * [0.0, 0.0]'; % x & y
   u([X1Y0_nod*dim - 1, X1Y0_nod*dim + 0]) = [strain_exp(1,i) strain_exp(3,i)/2 ; strain_exp(3,i)/2 strain_exp(2,i)] * [lx , 0.0]'; % x & y
   u([X1Y1_nod*dim - 1, X1Y1_nod*dim + 0]) = [strain_exp(1,i) strain_exp(3,i)/2 ; strain_exp(3,i)/2 strain_exp(2,i)] * [lx , ly ]'; % x & y
   u([X0Y1_nod*dim - 1, X0Y1_nod*dim + 0]) = [strain_exp(1,i) strain_exp(3,i)/2 ; strain_exp(3,i)/2 strain_exp(2,i)] * [0.0, ly ]'; % x & y
